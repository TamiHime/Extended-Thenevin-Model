...
app.post("/api/optimize", (req, res) => {
  ...
  exec(command, (error, stdout) => {
    ...
    if (!match) return res.status(500).json({ error: "Failed to parse output" });
    ...
    res.json({
      ...
      data: [{ time: 0, measured: 4.2, estimated: 4.1 }, { time: 1, measured: 4.1, estimated: 4.05 }],
      error: [{ time: 0, error: 5 }, { time: 1, error: 2 }]
    });
  });
});
